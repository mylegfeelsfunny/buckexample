...
open class Validation {
    ...
    fileprivate init() {
        ...
        let ssn = ValidationExpression(expression: "^([0-9]{3}[-]*[0-9]{2}[-]*[0-9]{4})+$", description: "Social Security Numbers", failureDescription: "Invalid SSN", hints: [
            ], interfaceBuilderAliases: ["ssn", "ss#"], transformText: nil, furtherValidation: nil)
        ...
        let birthdate = ValidationExpression(expression: "^([0-9]{2}[/]*[0-9]{2}[/]*[0-9]{4})+$", description: "Birthdate", failureDescription: "Invalid characters numbers present", hints: nil, interfaceBuilderAliases: ["positive numbers", "+"], transformText: nil, furtherValidation: nil)
        
        expressions = [zip, streetAddress, phone, email, ipAddress, MACAddress, GPSCoordinate, GPSPoint, URL, creditCard, money, letters, lettersWithSpaces, alphaNumeric, alphaNumericWithSpaces, positiveNumbers, negativeNumbers, wholeNumbers, positiveFloats, negativeFloats, allFloats, text, ssn, states, name, birthdate]
    ...
    open func isValid(_ validation: ValidationType, string: String) -> ValidationResult {
        if validation.rawValue == -1 {
            return ValidationResult(isValid: true, failureMessage: nil, transformedString: "")
        }
    ...
    func luhnTest(_ number: String) -> Bool {
        ...
        let reversedInts: [Int] = noSpaceNum.reversed().map { Int(String($0)) } .compactMap { $0 }
...
open class ValidationExpression {
    ...
    fileprivate var transformationClosure: ((String?) -> String)?
    fileprivate var furtherValidationClosure: ((String) -> ValidationResult)?
    ...
    public init(expression: String, description: String, failureDescription: String, hints: [ValidationRule]?, transformText: ((String?) -> String)?, furtherValidation: ((String) -> ValidationResult)?) {
        ...
        self.description = description
        self.hints = hints
        self.hints = self.hints?.sorted { item1, item2 in
    ...
    open func validate(_ string: String) -> ValidationResult {
        ...
        if isValid == false {
            if let h = hints, h.count > 0 {
                for index in 0...h.count - 1 {
                    let result = h[index].validate(self.transformedString)
...
open class ValidationResult {
    ...
    open var failureMessage: String?
    ...
    public var validationType: ValidationType {
        get {
            ...
            if rawvalue == nil {
                ...
                return ValidationType(rawValue: rawvalue as! Int)! // swiftlint:disable:this force_cast
            }
        }
        set {
            switch newValue {
